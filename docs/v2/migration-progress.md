...
- Branch `v2-monorepo`, working tree **clean**. Package manager is **pnpm**. If
  `node_modules` looks off, run `pnpm install` from the repo root.
- **Foundation F0–F2 are done and committed:**
  - F0 `828723c` — pnpm workspaces + Turborepo + `.npmrc` + `packages/config`.
  - F1 `72839d9` — `@needle/domain` + `@needle/contract` extracted; 31 imports rewired.
  - F2 `d83264f` — Electron app moved to `apps/desktop` (`@needle/desktop`); workspace
    root created; `.env` loader + one type fix. `turbo run typecheck`/`lint` pass.
- **F3 deferred to Track B** (premature without the web app — see Execution decisions).
- **Remaining this pass:** READMEs (step D), then push (step P).
- **Still needs a human:** launch the desktop app and confirm it runs — see "Final
  verification checklist". Known-good app is on `master`.
- **Current structure:** `apps/desktop` (Electron) · `packages/{domain,contract,config}`.
  Run the app: `pnpm install && pnpm start` (delegates to `@needle/desktop`).

**Update 2026-05-29:** Foundation pass is **complete and runtime-verified**. Steps D
(`fc56ed4`) and P are done; branch is pushed and up to date with `origin/v2-monorepo`.
The desktop app builds and boots from `apps/desktop` via `pnpm start` — forge preflight,
Vite main+preload, SQLite (no native-module error), `.env` API key load, and renderer
IPC (`app:getDiagnostics`, `app:getFlowHealth`) all confirmed via boot logs. Post-migration
fixes landed the same day: ⌘K crash (`ensureMainWindow`), pnpm 11 `nodeLinker: hoisted`
in `pnpm-workspace.yaml`, and cleanup of stale root `.vite/`/`dist/` plus an orphaned
pre-migration Electron process. **Still owner-verify:** pixel-level GUI interaction
(reorder/persist, full classify flow) — see "Final verification checklist".
...
|------|------|--------|--------|
| F0 | pnpm workspaces + Turborepo + `packages/config` + `.npmrc`; npm→pnpm | `pnpm run typecheck && pnpm run lint` | ✅ `828723c` |
| F1 | Extract `packages/domain` + `packages/contract` from `src/shared` | `pnpm run typecheck` | ✅ `72839d9` |
| F2 | Move Electron app → `apps/desktop`; fix forge/vite/tsconfig + `.env` | `turbo run typecheck lint` | ✅ `d83264f` |
| F3 | Extract `packages/ui-web` + `packages/design-tokens` | — | ⏸️ **deferred → Track B** |
| R  | Per-app `.cursor` rules/skills | — | ✅ convention documented (physical move deferred) |
| D  | READMEs (root + every folder) | n/a (docs) | ✅ `fc56ed4` |
| P  | Push `v2-monorepo` | `git push` | ✅ |

Each step is its own commit. To see them: `git log --oneline master..v2-monorepo`.

### Execution decisions (2026-05-28)

- **F3 deferred to Track B.** Extracting `ui-web`/`design-tokens` before the web app
  exists is premature: the component boundary and the transport-agnostic refactor
  should be driven by the real second consumer (web), not guessed now. The monorepo
  is already shaped to add these packages when web is built.
- **R is convention-only for now.** The per-app rules/skills convention (below) is
  documented so Track B agents scope rules correctly. The existing desktop `.cursor`
  rules/skills stay at the root for now (one app); when a second app lands, move the
  desktop-specific ones into `apps/desktop/.cursor/` (their globs then resolve
  relative to `apps/desktop`). Note: after F2 those root globs that say `src/...`
  now point at `apps/desktop/src/...`, so re-scope them when you do the move.
- **F2 verification gap:** `turbo run typecheck` + `lint` pass and preload bundles
  via Vite. The full Electron Forge build + GUI + SQLite runtime check is the human
  step (see "Final verification checklist"). The known-good app remains on `master`.

### 2026-05-29 runtime verification (Ofer directed, AI implemented)

- **Monorepo boot confirmed.** `pnpm start` from repo root (delegates to
  `@needle/desktop`) passes Electron Forge preflight (`Found pnpm@11.1.3`), Vite builds
  main+preload, `.env` loads `ANTHROPIC_API_KEY`, `[boot] ready` fires (v0.1.0,
  gitSha `fc56ed4`, `apiKeySource env`), and the renderer issues
  `app:getDiagnostics` + `app:getFlowHealth` IPC — window loaded, SQLite opened, no
  `better-sqlite3` native-module error.
- **⌘K crash fixed.** After closing the main window on macOS (app stays alive), ⌘K
  threw `TypeError: Object has been destroyed` because module-level `mainWindow` in
  `apps/desktop/src/main/index.ts` was never nulled on `closed`. Fix: `ensureMainWindow()`
  recreates the window when null/destroyed; `closed` handler nulls the reference; ⌘K
  and `activate` use the helper. Verified: `pnpm run typecheck` + `pnpm run lint` green.
- **pnpm 11 / Electron Forge blocker fixed.** Preflight failed: "node-linker must be set
  to hoisted". Root cause: `node-linker=hoisted` lived only in `.npmrc`, but **pnpm 11
  reads `nodeLinker` from `pnpm-workspace.yaml`, not `.npmrc`** — install had used the
  default isolated layout, which breaks Electron + native modules. Fix: added
  `nodeLinker: hoisted` to `pnpm-workspace.yaml`, removed ineffective
  `apps/desktop/.npmrc`, re-ran `pnpm install` (hoisted relink + native rebuild). Root
  `.npmrc` still has the old line (harmless, cleanup later).
- **Stale pre-migration artifacts cleaned up.** An orphaned Electron process from the
  old root build was still running; removed leftover root `.vite/` and `dist/` (canonical
  build is now `apps/desktop/.vite`). The original ⌘K error had come from that stale
  root build path.
...
- If native modules misbehave at runtime: confirm **`nodeLinker: hoisted` is set in
  `pnpm-workspace.yaml`** (pnpm 11 ignores `.npmrc` for this — see 2026-05-29 note
  above), then `pnpm install` again. Last resort: fall back to npm workspaces.
...
After the pass, confirm the runtime (agent verified boot logs 2026-05-29; GUI
interaction items still for Ofer):

```bash
pnpm install
pnpm --filter @needle/desktop start     # app window opens
```
- [x] App launches (window appears). *Confirmed 2026-05-29: forge preflight, Vite
  build, `[boot] ready`, renderer IPC.*
- [x] Today view loads data (SQLite read works → native module OK). *Confirmed
  2026-05-29: SQLite opened, no native-module error; `app:getFlowHealth` IPC.*
- [ ] Add/reorder a task; restart; data persists (SQLite write works). *Owner GUI
  verify.*
- [ ] Capture → AI classify returns blocks. *`.env` key load confirmed (`apiKeySource
  env`); full classify flow owner-verify.*
- [x] `pnpm run typecheck` and `pnpm run lint` pass from the repo root.
